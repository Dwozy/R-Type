...
namespace RType::Client
{
    ...
    void RTypeClient::updateOtherPlayerMovement(RType::Protocol::MoveData entity)
    {
        ...
        if (!_searchEntity(entity.id))
            return;
        id = _findEntity(entity.id);
        std::cerr << "entity: " << entity.id << " before if" << std::endl;
        if (!transforms[id].has_value() && entity.id == _serverId)
            return;
        std::cerr << "entity: " << entity.id << " after if" << std::endl;
        transforms[id]->position = {entity.x, entity.y};
        transforms[id]->velocity = {entity.dx, entity.dy};
    }
} // namespace RType::Client
