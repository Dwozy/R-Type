...
{
    void RTypeClient::setCollisionInformation(
        struct RType::Protocol::CollisionData collisionData, GameEngine::Entity &entity)
    {
    ...
    void RTypeClient::getCollisionInformation(struct RType::Protocol::CollisionData collisionData)
    {
        ...
        if (!searchEntity(collisionData.id)) {
            ...
            setCollisionInformation(collisionData, entity);
        }
        id = findEntity(collisionData.id);
        if (!collisions[id]) {
