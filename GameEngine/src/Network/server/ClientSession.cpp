...
#include "Network/Server/ClientSession.hpp"

namespace GameEngine::Network
{
    ClientSession::ClientSession(asio::io_context &IOContext, SafeQueue<std::string> &clientsMessages)
    ...
    std::shared_ptr<ClientSession> ClientSession::get() { return shared_from_this(); }

    void ClientSession::handleWrite(const asio::error_code &error)
    {
        if (!error) {
            _socket.async_read_some(asio::buffer(_readBuffer),
                std::bind(&ClientSession::handleRead, get(), std::placeholders::_1, std::placeholders::_2));
        } else {
            std::cerr << error.message() << std::endl;
        }
    }

    void ClientSession::handleRead(const asio::error_code &error, std::size_t transferredBytes)
    {
        if (!error) {
            std::string message;
            ...
            asio::async_write(_socket, asio::buffer(_readBuffer, transferredBytes),
                std::bind(&ClientSession::handleWrite, get(), std::placeholders::_1));
        } else {
