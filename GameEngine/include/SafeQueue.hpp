...
class SafeQueue : public ISafeQueue<T>
{
  public:
    ...
    /// @brief Try to pop a value from the queue, lock a mutex while trying to
    /// pop the value
    /// @param value Reference to be filled withthe popped value
    /// @return true if the value was successfully popped, false if there was no
    /// value in queue
    ...
    /// @brief Pop a value from the queue, lock a mutex and wait if the queue is
    /// empty
