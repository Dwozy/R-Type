...
Network::UdpServer::UdpServer(asio::io_context &IOContext, int port,
    ...
    : _socket(IOContext, asio::ip::udp::endpoint(asio::ip::udp::v4(), port)),
      ...
      _clientsMessages(clientsMessages), _buffer(_streamBuffer.prepare(1))
{
    receive();
    updateGameInfo();
...
Network::UdpServer::~UdpServer()
{
    ...
    for (std::pair<unsigned short, asio::ip::udp::endpoint> client :
         _listClient) {
        _communication.sendData(message.data(), message.size(),
                                static_cast<uint8_t>(rtype::PacketType::STRING),
...
void Network::UdpServer::receive()
{
    ...
    _socket.async_receive_from(
        asio::buffer(_buffer), _clientEndpoint,
        ...
        for (std::pair<unsigned short, asio::ip::udp::endpoint> client :
            ...
            if (buff == "Room\n") {
                ...
                _communication.sendData(
                    byteArrayToSend.data(), byteArrayToSend.size(),
                    static_cast<uint8_t>(rtype::PacketType::ROOM), _socket,
                    client.second);
            } else
                _communication.sendData(
                    buff.data(), buff.size(),
                    static_cast<uint8_t>(rtype::PacketType::STRING), _socket,
                    client.second);
        }
    }
