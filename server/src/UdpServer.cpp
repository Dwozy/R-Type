...
Network::UdpServer::~UdpServer()
{
    ...
    for (std::pair<unsigned short, boost::asio::ip::udp::endpoint> elem :
         _listClient) {
        _socket.send_to(boost::asio::buffer(message.c_str(), message.length()),
                        elem.second);
    }
    _socket.close();
}

void Network::UdpServer::handleReceive(const boost::system::error_code &error,
...
void Network::UdpServer::receive()
{
    ...
    _socket.async_receive_from(boost::asio::buffer(_readBuffer, 1024),
                               _clientEndpoint,
...
void Network::UdpServer::updateTCPInformation()
{
    ...
    while (_clientsMessages.tryPop(buff)) {
        for (std::pair<unsigned short, boost::asio::ip::udp::endpoint> client :
             _listClient)
            _socket.send_to(boost::asio::buffer(buff.c_str(), buff.length()),
                            client.second);
    }
    _timerTCP.expires_from_now(boost::posix_time::seconds(0));
    _timerTCP.async_wait(
        boost::bind(&Network::UdpServer::updateTCPInformation, this));
}
