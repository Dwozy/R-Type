...
Network::TcpServer::TcpServer(asio::io_context &IOContext, int port, SafeQueue<std::string> &clientsMessages)
...
void Network::TcpServer::accept(std::shared_ptr<ClientSession> clientSession)
{
    _acceptor.async_accept(clientSession->getSocket(),
        std::bind(&Network::TcpServer::handleAccept, this, clientSession, std::placeholders::_1));
}

Network::TcpServer::~TcpServer() { _socket.close(); }

void Network::TcpServer::handleAccept(std::shared_ptr<ClientSession> clientSession, const asio::error_code &error)
{
    if (!error) {
