...
TcpClient::TcpClient(boost::asio::io_context &IOContext, boost::asio::ip::tcp::endpoint &serverEndpoint):
...
void TcpClient::handleInput(const boost::system::error_code &error, std::size_t recvBytes)
{
    if (!error) {
        boost::asio::streambuf::const_buffers_type bufs = _inputBuffer.data();
        std::string str(boost::asio::buffers_begin(bufs), boost::asio::buffers_begin(bufs) + recvBytes);
...
void TcpClient::handleWrite(const boost::system::error_code &error)
{
    if (!error) {
        boost::asio::async_read_until(_input, _inputBuffer, '\n',
            boost::bind(&TcpClient::handleInput, this, boost::placeholders::_1, boost::placeholders::_2));
    } else if (error == boost::asio::error::operation_aborted) {
